...
! through the command line. Below is info on how to compile (with gfortran) and run the program.
!
...
! Compile/run on Linux:
!
! > gfortran -o advection_solver advection_solver.f90
! > ./advection_solver <method_type> <differentation_type> <output_filename>
!
...
PROGRAM advection_solver
    ...
    double precision, parameter :: epsilon = 1E-15                  ! Small to avoid zero division in MPDATA
    ...
    do n = 0, n_max     ! Loop, in time as long as necessary to complete requested number of cycles
        theta(1, n_curr) = theta(j_max, n_curr)     ! BC theta at the left boundary x = 0
        ...
        if (method_type == "mpdata") then   ! Employ MPDATA method when specified
            do i = 1, num_iterations        ! Use MPDATA for num_iterations iterations
                ...
                ! Compute u_star (at the j_max using BC) if theta_star > 0 with requested FDA.
                ...
                ! Scale anti-diffusive velocity at end point j_max.
                if (use_scaling .eqv. .TRUE.) then
                    u_star(j_max) = u_star(j_max) * S_c
                end if
                ...
                if (use_scaling .eqv. .TRUE.) then
                    exit    ! Exit iteration loop after one iteration if scaling method is used
                end if
            end do
        end if
